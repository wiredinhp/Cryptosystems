...
import SPN_algorithm as cipher
...
sbox_out = [ bin(cipher.s_box[int(seq,2)])[2:].zfill(4) for seq in sbox_in ]
...
for pt in range(10000):
    ct = cipher.encrypt(pt, bin(int(k,16))[2:])
    ct_5_8 = (ct>>8)&0b1111
    ct_13_16 = ct&0b1111
    ...
    for target in range(256):
        ...
        u_5_8, u_13_16 = cipher.s_box_inv[v_5_8], cipher.s_box_inv[v_13_16]
